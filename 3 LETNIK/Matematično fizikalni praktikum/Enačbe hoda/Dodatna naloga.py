...
def f1( y_0, t ):
#        return x * numpy.sin( t )
    return -k*(y_0-y_zunaj)+ A1*np.sin(2*np.pi/24*(t-d))
t_rkfA1, x_rkfA1 = rkf( f1, 0, lenght_T, y_0, h, 200*h, h )

A2=2
def f2( y_0, t ):
#        return x * numpy.sin( t )
    return -k*(y_0-y_zunaj)+ A2*np.sin(2*np.pi/24*(t-d))
t_rkfA2, x_rkfA2 = rkf( f2, 0, lenght_T, y_0, h, 200*h, h )

A3=3
def f3( y_0, t ):
#        return x * numpy.sin( t )
    return -k*(y_0-y_zunaj)+ A3*np.sin(2*np.pi/24*(t-d))
t_rkfA3, x_rkfA3 = rkf( f3, 0, lenght_T, y_0, h, 200*h, h )


A05=0.5
def f4( y_0, t ):
#        return x * numpy.sin( t )
    return -k*(y_0-y_zunaj)+ A05*np.sin(2*np.pi/24*(t-d))
t_rkfA05, x_rkfA05 = rkf( f4, 0, lenght_T, y_0, h, 200*h, h )

A02=0.2
def f5( y_0, t ):
#        return x * numpy.sin( t )
    return -k*(y_0-y_zunaj)+ A02*np.sin(2*np.pi/24*(t-d))
t_rkfA02, x_rkfA02 = rkf( f5, 0, lenght_T, y_0, h, 200*h, h )

A5=5
def f6( y_0, t ):
#        return x * numpy.sin( t )
    return -k*(y_0-y_zunaj)+ A5*np.sin(2*np.pi/24*(t-d))
t_rkfA5, x_rkfA5 = rkf( f6, 0, lenght_T, y_0, h, 200*h, h )

plt.plot(t_rkfA02,x_rkfA02,'-', label="A=0.2")
plt.plot(t_rkfA05,x_rkfA05,'-', label="A=0.5")
plt.plot(t_rkfA1,x_rkfA1,'-', label="A=1")
plt.plot(t_rkfA3,x_rkfA3,'-', label="A=3")
plt.plot(t_rkfA5,x_rkfA5,'-', label="A=5")
#plt.plot(t,y_main,'-', label="Dejanska funkcija")
plt.title('Prikaz Runge-Kutta-Fehlberg funkcije v odvisnosti od parametra A')
plt.xlabel("t")
plt.ylabel("T(t)")
#plt.axis([-1,75, -7,25])
plt.legend()
