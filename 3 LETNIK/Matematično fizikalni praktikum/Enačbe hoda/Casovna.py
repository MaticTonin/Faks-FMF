...
def izdelava_grafa(N_MAX, lenght_T):
    h=lenght_T/N_MAX
    t=linspace(0,lenght_T,N_MAX)
    ...
    return t, Euler, Simetric, Heun, Midpoint, RK4
def f( y_0, t ):
...
def casovna(N_MAX):
    ...
    while n<N_MAX:
        ...
        startRKF = time.time()
        RKF=rkf( f, 0, lenght_T, y_0, h, 200*h, h )
        endRKF= time.time()

        n_t.append(n)

        Eul.append(endE-startE)
        Sim.append(endS-startS)
        He.append(endH-startH)
        Mid.append(endM-startM)
        RK.append(endRK-startRK)
        rkf1.append(endRKF-startRKF)
        n+=50000

    return n_t, Eul, Sim, He, Mid, RK, rkf1
...
casi=casovna(1000000)
print(casi[2])
plt.plot(casi[0],casi[1],'-',color="blue", label="Eulerjeva")
plt.plot(casi[0],casi[2],'-',color="green",label="Eulerjeva simetrična")
plt.plot(casi[0],casi[3],'-',color="purple", label="Heunova")
plt.plot(casi[0],casi[4],'-',color="orange", label="Midpoint")
plt.plot(casi[0],casi[5],'-',color="pink", label="Runge Kutta 4")
plt.plot(casi[0],casi[6],'-',color="red", label="Runge Kutta Fehlberg")
plt.title('Časovna zahtevnost metode z večanjem korakov')
plt.xlabel("N")
plt.ylabel("t")
plt.legend()
plt.show()
