...
#Katera metoda je boljša 1 ali 2
k=100
točke=0
int1=[]
int2=[]
...
m=5000
t1=np.linspace(0, (delta- (delta/m)),m)
sin1=np.sin(t1)
fftsin2= np.fft.fftshift(Fourier2(sin1,m))
met1=InverseFourier(Fourier(sin1,m),m)
met2=InverseFourier2(np.fft.fftshift(fftsin2),m)

odstop1=[]
odstop2=[]
for i in range(m):
    print(i)
    odstop1.append(abs(sin1[i]-met1[i]))
    odstop2.append(abs(sin1[i]-met2[i]))


plt.plot(t1,met1,'-',color="red", label="Odstopanje z izdelavo vektorjev")
plt.plot(t1,met2,'-',color="blue", label="Odstopanje z izdelavo vsote")
plt.title('Graf odstopanja inverzne transformacije za sin(x)')
plt.xlabel("t")
plt.ylabel("INF(f(x))")
plt.legend()
plt.show()  
plt.plot(t1,odstop1,'-',color="red", label="Odstopanje z izdelavo vektorjev")
plt.plot(t1,odstop2,'-',color="blue", label="Odstopanje z izdelavo vsote")
plt.title('Graf odstopanja inverzne transformacije za sin(x)')
plt.xlabel("t")
plt.ylabel("INF(f(x))")
plt.legend()
plt.show()  
#Plotanje grafov za Forierja
start1 = time.time()
