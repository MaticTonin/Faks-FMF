...
if plot_Matrix==True:
    plt.subplot(2,1,1)
    plt.suptitle(r"Prikaz časovnega razvoja valovne funkcije, $\lambda=%.2f, \tau=%.4f, h=%.3f$" %(lam,tau,h))
    plt.title(r"$|\Psi(x)|^2$")
    plt.xlabel("x")
    plt.ylabel("t")
    plt.xticks([0, M/2, M-1], [-L,0,L])
...
if plot_animation==True:
    ...
    line1, = ax.plot(x, Wawe0[0,:N].real, color="red", label=r"Real($\Psi(x))$, implicitna")
    line2, = ax.plot(x, Wawe2[0,:N].real, color="green", label=r"Real($\Psi(x))$, propagator")
    line3, = ax.plot(x, abs(Wawe0[0,:N]), color="blue", label=r"$|\Psi(x)|^2$, implicitna")
    line4, = ax.plot(x, abs(Wawe2[0,:N]), color="black", label=r"$|\Psi(x)|^2$, propagator")
