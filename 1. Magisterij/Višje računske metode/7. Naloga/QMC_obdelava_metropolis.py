...
from qmc import qmc_move, qmc_eval
reps = int(1e+2)
steps=int(1e+5)
    ...
    if beta_plot=="Yes":
        ...
        for j in range(len(M_list)):
            ...
            for i in tqdm(range(len(beta_list))):
                ...
                model = Metropolis(np.random.normal(size=M), qmc_move, qmc_eval, [1,0], rep=reps)
                ep, extra, Energy_list, H_list, V_list ,epsilon = model.run(beta_list[i], steps)[1:]
                print(ep)
                E_mean.append( np.mean(np.array(ep)[int(steps/2):steps]) )
                E_std.append( np.std(np.array(ep)[int(steps/2):steps]) )
                H_mean.append( np.mean(np.array(extra)[:,0][int(steps/2):steps]) )
                H_std.append( np.std(np.array(extra)[:,0][int(steps/2):steps]) )
                V_mean.append( np.mean(np.array(extra)[:,1][int(steps/2):steps]) )
                V_std.append( np.std(np.array(extra)[:,1][int(steps/2):steps]) )
