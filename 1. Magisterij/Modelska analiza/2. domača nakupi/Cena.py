...
A_ub = np.array([-i[0:8] for i in data])
...
A_ub = np.concatenate((A_ub, np.array([[i[8] for i in data]]).T), axis=1)
for i in data:
    print(sum(i[1:4])+0.001*sum(i[5:8]))
print(A_ub)
b_ub = np.array([2000, -70, -310, -50, -1000, -18, -60, -3500, 2, -500, 2400])
...
plt.title("Optimizacija cene, zgolj relavantne količine")
...
plt.show()
legend=["Cena: %.2f" %(sum_price), "Kalorije: %.2f" %(cal_sum)]
plt.pie(resoult, labels = [names[int(i), 0] for i in indexation], autopct='%1.1f%%')
plt.xlabel("živilo")
plt.legend(legend)
plt.title("Optimalna razporeditev hrane za najmanjšo ceno, brez zelja in ovsenih kosmičev")
# show plot
plt.show()
...
plt.title("Optimalna razporeditev hrane za najmanjšo ceno, snovi, , brez zelja in ovsenih kosmičev")
plt.subplot(2,1,2)
plt.pie([Ca_sum,Fe_sum+C_sum,K_sum,Na_sum], labels = ["Ca",	"Fe+ Vitamin C",	"K", "Na"], autopct='%1.1f%%')
plt.title("Optimalna razporeditev hrane za najmanjšo ceno, minerali, , brez zelja in ovsenih kosmičev")
# show plot
plt.show()