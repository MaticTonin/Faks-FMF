...
for i in tqdm(range(try_list)):
    ...
    ax1.plot(tlist,Nlist,".-", color="blue", alpha=0.009)
...
ax1.set_title("Prikaz več generiranih populacijskih modelov z dodatkom rodnosti")
...
for b_1 in b_list:
    ...
    for i in tqdm(range(try_list)):
        ...
        Nlist1, tlist1=model_2(N,dt,b_1)
        color = cm.rainbow(np.linspace(0, 1, len(Nlist)))
        t_death1.append(tlist[len(tlist)-1])
        t_death2.append(tlist1[len(tlist1)-1])
    if b_1!=10:
        n, bins, patches =plt.hist(t_death1,bins = int(np.amax(t_death1)*2), label = r'Rodnost, $\beta = $'+ str(b_1), color = plt.cm.Blues(index/len(b_list)),alpha = 0.8)
        n, bins, patches =plt.hist(t_death2,bins = int(np.amax(t_death2)*2), label = r'Normal, $\beta = $'+ str(b_1), color = plt.cm.Reds(index/len(b_list)),alpha = 0.8)
    index+=1
plt.title(r"Prikaz primerjave modelov za različne $\beta$ pri $N=%i$"%(N))
