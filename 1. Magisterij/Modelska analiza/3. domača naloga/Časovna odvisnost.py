...
for i in metode:
    ...
    for j in range(2,10):
        ...
        razlika.append(np.log(abs(data.T[j-2]-energija)))
    plt.plot(x,cas, label= "Metoda"+str(i))
    plt.title("Prikaz časovne zahtevnosti alguritma")
    plt.legend()
    plt.show()
    plt.title("Prikaz odstopanja alguritma")
    plt.plot(x,razlika)
    plt.show()
