...
def creating(N,p):
    ...
    for i in range(int(N)):
        ...
        if((np.sqrt(np.abs(x)) + np.sqrt(np.abs(y)) + np.sqrt(np.abs(z))) <= 1):
            n = n + 1*np.sqrt((x**2 + y**2 + z**2))**p*(x**2+y**2)
            x_line.append(x)
            y_line.append(y)
            z_line.append(z)
            color.append(i)
...
for i in tqdm(p_line):
    ...
    t_tot+=end-start
    timer.append(t_tot)
    #plt.errorbar(i, 8*n/10**N,sigma, fmt ='.b', capsize=3)
plt.plot(p_line, masa, "x-")
plt.title("Prikaz odvisnosti $J_{z,z}$ od izbire p, $N=10^{%i}$" %(N))
plt.xlabel("p")
plt.ylabel("m")
plt.legend()
plt.grid()
