...
plt.title(r"Prikaz histograma porazdelitve delcev od števila sipanj za srednje $\nu$")
plt.hist(št_sipanj_list[:,0], color = plt.cm.gnuplot(3/3), alpha=1, bins=6,label = r'$\nu$ = '+str(nu_list[0]))
plt.hist(št_sipanj_list[:,10], color = plt.cm.gnuplot(1/3),alpha=0.5,bins=4, label = r'$\nu$ = '+str(nu_list[10]))
plt.legend()
plt.grid()
...
plt.scatter(nu_list,T_line, label="Prepustnost T", color="blue")
plt.scatter(nu_list,R_line, label="Odbojnost R", color="red")
plt.legend()
plt.xlabel(r"$\nu$")
plt.ylabel("T/R")
plt.grid()
...
plt.axvline(x = 0.01, color = 'gray', label = 'axvline - full height')
plt.text(0.12, max(odboji_line)/2, "Nizke", color='black')
plt.axvspan(0.01, 0.1, alpha=0.2, color='black')
plt.axvline(x = 0.1, color = 'gray', label = 'axvline - full height')
...
plt.axvline(x = 2, color = 'red', label = 'axvline - full height')
