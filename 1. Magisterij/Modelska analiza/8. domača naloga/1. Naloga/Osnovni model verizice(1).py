...
def izdelava_verizce_plot(Nivoji,N,a,kT,Energy,cleni):
    verizica=np.zeros(cleni)
    verizica[0]=0
    verizica[cleni-1]=0
    E = 0
    ...
    for i in range(N):
        ...
        if dE>0:
            ...
            if zeta<=np.exp(-dE/kT):
                verizica[place]=verizica[place]+delta_i
        if i==0:
            ax1.plot(Mulekle,verizica, ".-",label="Začetna", color="blue")
        if i!=(N-1) and i%20:
            ax1.plot(Mulekle,verizica,".-",alpha=0.01, color=plt.cm.rainbow(i/N))
            
    return verizica, E

N_list=np.linspace(10,10**5,30)
Mulekle=[]
...
verizica,energija=izdelava_verizce_plot(Nivoji,int(N),a,kT,Energy,cleni)
ax1.plot(Mulekle,verizica,".-", label="Končna", color="black") 

ax1.set_title(r"Konvergenca verižice $k_bT=%.2f$ pri $\alpha=%.2f$" %(kT,a))
ax1.legend()
...
for N in tqdm(N_list):
    ...
    if jndex==len(N_list)-1:
        ax1.plot(Mulekle,verizica,".-",label=r"$k_bT=%.2f$" %(kT), color=plt.cm.rainbow(len(kT_list)/len(kT_list)))
    else: 
        ax1.plot(Mulekle,verizica,".-",alpha=0.20, color=plt.cm.rainbow(0/len(kT_list)))
    jndex+=1
    
ax1.set_title(r"Prikaz verižice za $k_bT=%.2f$ pri $\alpha=%.2f$ za različne N" %(kT,a))
ax1.legend()
ax1.set_xlabel("Mulekule")
ax1.set_ylabel(r"h")
plt.show()      
...
for Nivoji in tqdm(Nivoji_list):
    ...
    for N in N_list:
        ...
        if jndex==len(N_list)-1 and index%10==0:
            ...
            ax7.plot(Mulekle,verizica,".-",alpha=0.01, color=plt.cm.rainbow(index/len(Nivoji_list)))
        jndex+=1
