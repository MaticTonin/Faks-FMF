...
# 
# V splošnem števila dajemo na sklad, z operacijo pa s sklada vzamemo
# dve vrhnji števili, nanj pa vrnemo rezultat operacije.
# =====================================================================@005669=
# 1. podnaloga
...
# =====================================================================@005670=
# 2. podnaloga
# Sestavite funkcijo `rpn(niz)`, ki dobi niz z veljavnim
# RPN izrazom ter izračuna in vrne njegovo vrednost.
# 
#     >>> rpn('10 5 + 3 7 * +')
#     36
# 
...
def rpn(niz):
    delcek=[]
    for i in niz.strip().split(" "):
        if i in operacije:
            delcek.append(izracunaj(delcek.pop(), delcek.pop(), i))
        else:
            delcek.append(i)
    return int(delcek[0])
...
# =====================================================================@005671=
# 3. podnaloga
# Napišite funkcijo `obicajni_zapis(niz)`, ki sprejme izraz v RPN in vrne
...
#     '((10 + 5) + (3 * 7))'
#  
# Namig: Ali lahko nalogo rešite tako, da minimalno predelate funkcijo `rpn`?
# =============================================================================
def zdruzi(a,b,op):
    return "(" + b + " " + op + " " + a + ")"
...
def obicajni_zapis(niz):
    ...
    for i in niz.strip().split(" "):
        if i in operacije:
            racun.append(zdruzi(racun.pop(),racun.pop(), i))
        else:
            racun.append(i)
    return racun[0]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTQ2LCJwYXJ0Ijo1NjY5fQ:1iIycY:kkX12aVMDEATFLl8vExXJkEPrVg'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTQ2LCJwYXJ0Ijo1NjcwfQ:1iIycY:5-KJCRuYr2LPCTk0Xe4zsk1c5v8'
        try:
            Check.equal("rpn('2 4 +')", 6)
            Check.equal("rpn('2 4 + 3 *')", 18)
            Check.equal("rpn('10 5 + 3 7 * +')", 36)
            Check.equal("rpn('1 3 5 7 9 + * + *')", 83)
            Check.equal("rpn('1 3 5 7 9 + * + * 2 4 6 8 10 + * + * +')", 307)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTQ2LCJwYXJ0Ijo1NjcxfQ:1iIycY:TDUdrrIp8pofrk9zaVfD4sCu9gw'
        try:
