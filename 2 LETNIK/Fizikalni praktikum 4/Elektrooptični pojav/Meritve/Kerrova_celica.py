...
my_file = os.path.join(THIS_FOLDER, "1polarizator.txt")
...
def P(x, P0, P1, D):
    return P1*(np.sin(x+D))**2 + P0
args = [0, 0, 0]
fit = sp.curve_fit(P, x, y, p0=args, sigma=yerror)
print(fit)
fitlabel = "$(%.4f \pm %.3f) \cdot sin^2(\\alpha + %.4f \pm %.3f) + %.4f \pm %.3f$"%(fit[0][1], np.sqrt(fit[1][1][1]), fit[0][2], np.sqrt(fit[1][2][2]), fit[0][0], np.sqrt(fit[1][0][0]))
print(fitlabel)
...
plt.savefig("Izkoristek_kot,1_polarizator.png")
plt.show()
plt.close()
