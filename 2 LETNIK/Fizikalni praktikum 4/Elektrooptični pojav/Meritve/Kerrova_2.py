...
def P(x, P1, B, D_0):
    return P1*(np.sin(B*3.14*L*x**2/d**(2)+D_0))**2
args = [0, 0, 0]
fit = sp.curve_fit(P, x, y, p0=None, sigma=None)
print(fit)
fitlabel = "$(%.4f *sin( np.pi *\\frac{L}{d^2}x^2 \cdot %.4E + %.4f)$" %(fit[0][0], fit[0][1], fit[0][2])
print(fitlabel)
t0 = np.linspace(x[0], 1050, 1000)
print(fit[0][1])
