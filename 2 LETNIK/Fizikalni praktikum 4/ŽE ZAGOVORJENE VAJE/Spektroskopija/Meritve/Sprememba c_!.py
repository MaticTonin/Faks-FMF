...
for i in range(len(pi1_Ne)):
    ...
    lam51.append(((-7.6056+np.sqrt(7.6056**2+4*0.135*(69.7-pi1_Ne[i])))/(-2*0.135))**2)
for i in range(len(lam5)):
    print("Valovna dolžina \u03C6_1 je: %.4f nm" %lam5[i])
print("\n")
print("Barva & Tablična $\lambda$ & $\lambda$ & Napaka $\lambda$ \\\ ")
for i in range(len(lam5)):
    print(" // & %.2f nm & %.2f nm & %.2f nm  \\\ " %(lam1[i], lam5[i], lam51[i]))
    print("\\hline")
print("\n")
